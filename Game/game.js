...
import { getPickedCards } from "./storage.js";
...
import { makeDraggable } from "./system/makedraggable.js";

/* ===== NEW: PROGRESS & WIN ===== */
import { markLevelCleared, isLevelUnlocked, isLevelCleared } from "./system/progress.js";
import { showOverlay } from "./system/win.js"; // Combined win/lose overlay
export function showGame(container, level, onExitToLobby) {
  /* ===== GLOBAL NO-SCROLL ===== */
  document.body.style.margin = "0";
  document.body.style.overflow = "hidden";
  container.style.overflow = "hidden";
  let gameLocked = true; // prevents movement until cards are done
...
startBtn.addEventListener("click", () => {
  if (gameLocked) return; // ⛔ blocked until card event ends
...
// ===== CARD EVENT AT START OF LEVEL (EXCEPT LEVEL 1) =====
if (level !== 1) {
  showCardEvent(wrapper);

  const cardObserver = new MutationObserver(() => {
    const cardOverlayExists = wrapper.querySelector(
      'div[style*="rgba(0,0,0"]'
    );

    if (!cardOverlayExists) {
      console.log(`[CARD EVENT] Finished for level ${level}`);
      gameLocked = false;
      cardObserver.disconnect();
    }
  });

  cardObserver.observe(wrapper, { childList: true, subtree: true });

} else {
  // Level 1 skips card event
  console.log("[CARD EVENT] Skipped for level 1");
  gameLocked = false;
}
